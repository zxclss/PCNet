...
class BrierFDE(Metric):
    ...
    def __init__(self,
                 max_guesses: int = 6,
                 **kwargs) -> None:
        super(BrierFDE, self).__init__(**kwargs)
        self.add_state('sum', default=torch.tensor(0.0), dist_reduce_fx='sum')
        self.add_state('count', default=torch.tensor(0), dist_reduce_fx='sum')
        self.max_guesses = max_guesses
    ...
    def update(self,
        ...
        pred_topk, prob_topk = topk(self.max_guesses, pred, prob)
        inds_last = (valid_mask * torch.arange(1, valid_mask.size(-1) + 1, device=self.device)).argmax(dim=-1)
        min_fde, min_idx = torch.norm(pred_topk[torch.arange(pred.size(0)), :, inds_last] -
                                target[torch.arange(pred.size(0)), inds_last].unsqueeze(-2),
                                p=2, dim=-1).min(dim=-1)
        self.sum += ((1.0 - prob_topk[torch.arange(pred.size(0)), min_idx]) ** 2 + min_fde).sum()
        self.count += pred.size(0)
